/**
 * Child-directive walking for `schema`'s container components, adapted from
 ...
 * (a `schema_stage`, say) is the SAME shared element type until React
 ...
 * package. This module reimplements the same pattern for `schema`'s own
 ...
 * directive was written with bracket content (`::schema_stage[Ingest]`) or
 * not (`::schema_stage{name="Ingest"}`), since a leaf directive with no
