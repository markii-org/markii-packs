...
const STAGE_NAME = 'schema_stage';

/**
 * `:::schema_pipeline{title="..."}` ... `:::`, holding a run of
 * `::schema_stage` children: a horizontal flow with an arrow drawn between
 * each pair of stages.
 *
 ...
 * the same `StageChip` markup `::schema_stage` uses on its own, so nothing
...
export function DataPipeline({ attributes, children }: MarkComponentProps): ReactElement {
  ...
  return (
    <div className="mk-schema_pipeline">
      {title !== '' && <div className="mk-schema_pipeline__title">{title}</div>}
      {stages.length > 0 ? (
        <div className="mk-schema_pipeline__flow">
          {stages.map((stage, index) => (
            <span className="mk-schema_pipeline__step" key={index}>
              {index > 0 && (
                <span className="mk-schema_pipeline__arrow" aria-hidden="true">
                  &#8594;
                </span>
              )}
              <StageChip
