...
export function DataColumn({ attributes }: MarkComponentProps): ReactElement {
  ...
  return (
    <div className="mk-schema_col">
      <code
        className={
          name !== '' ? 'mk-schema_col__name' : 'mk-schema_col__name mk-schema_col__name--placeholder'
        }
      >
        {name !== '' ? name : 'unnamed column'}
      </code>
      {isKey && (
        <span className="mk-schema_col__pk" title="primary key">
          PK
        </span>
      )}
      {type !== '' && <span className="mk-schema_col__type">{type}</span>}
      {note !== '' && <span className="mk-schema_col__note">{note}</span>}
    </div>
  );
}
