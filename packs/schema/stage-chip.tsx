...
/**
 * The chip markup for one pipeline stage, shared by `::schema_stage` (which
 * renders one on its own) and `:::schema_pipeline` (which rebuilds the flow
...
export function StageChip({
  ...
  return (
    <span className="mk-schema_stage" style={minWidth !== undefined ? { minWidth } : undefined}>
      <span
        className={
          hasLabel ? 'mk-schema_stage__label' : 'mk-schema_stage__label mk-schema_stage__label--placeholder'
        }
      >
        {label}
      </span>
      {tech !== '' && <span className="mk-schema_stage__tech">{tech}</span>}
    </span>
  );
}
