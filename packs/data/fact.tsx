...
/**
 * `::data_fact[default shuffle partitions]{value=200}`, a figure worth
...
export function DataFact({ attributes, children }: MarkComponentProps): ReactElement {
  ...
  if (value === '' && !hasCaption) {
    return (
      <div className="mk-data_fact">
        <span className="mk-data_fact__empty">no figure recorded</span>
      </div>
    );
  }
  ...
  return (
    <div className="mk-data_fact">
      {value !== '' && <span className="mk-data_fact__value">{value}</span>}
      {hasCaption && <span className="mk-data_fact__label">{caption}</span>}
    </div>
  );
}
