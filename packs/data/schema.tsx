...
/**
 * `:::data_schema{name="events_raw"}` ... `:::`, a schema card for one
 * table or dataset: a monospace name strip, then the column list below.
 *
 * The body is normally a run of `::data_col` children, but this component
...
export function DataSchema({ attributes, children }: MarkComponentProps): ReactElement {
  ...
  return (
    <div className="mk-data_schema">
      <div className="mk-data_schema__head">
        <span
          className={
            name !== ''
              ? 'mk-data_schema__name'
              : 'mk-data_schema__name mk-data_schema__name--placeholder'
          }
        >
          {name !== '' ? name : 'unnamed dataset'}
        </span>
      </div>
      <div className="mk-data_schema__body">{children}</div>
    </div>
  );
}
