/**
 * Shared defensive helpers for the `track` pack.
 *
 ...
 * to guard. Every `track_*` component therefore routes all extraction of
 * its bound `data` through `safeExtract`, only ever lets primitives
 * (strings and finite numbers) and plain arrays/objects built from them
 * escape the guard, and surfaces a failed or stale binding exactly the way
 * the reference components do: a `title` tooltip and a CSS modifier class,
 * never body text.
 */
...
/** Coerces an unknown value to a finite number, or `fallback`. */
 ...
 * bound data (strings, numbers, plain arrays/objects of them), never the
 * hostile object itself; `fallback` must not touch the bound value at all.
 */
